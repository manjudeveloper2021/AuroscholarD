...
final class BitmapImageUtils {
  ...
  static Bitmap resizeBitmap(
          Bitmap bitmap, int reqWidth, int reqHeight, CropImageViews.RequestSizeOptions options) {
    try {
      if (reqWidth > 0
          && reqHeight > 0
          && (options == CropImageViews.RequestSizeOptions.RESIZE_FIT
              || options == CropImageViews.RequestSizeOptions.RESIZE_INSIDE
              || options == CropImageViews.RequestSizeOptions.RESIZE_EXACT)) {

        Bitmap resized = null;
        if (options == CropImageViews.RequestSizeOptions.RESIZE_EXACT) {
          resized = Bitmap.createScaledBitmap(bitmap, reqWidth, reqHeight, false);
        } else {
          int width = bitmap.getWidth();
          int height = bitmap.getHeight();
          float scale = Math.max(width / (float) reqWidth, height / (float) reqHeight);
          if (scale > 1 || options == CropImageViews.RequestSizeOptions.RESIZE_FIT) {
            resized =
