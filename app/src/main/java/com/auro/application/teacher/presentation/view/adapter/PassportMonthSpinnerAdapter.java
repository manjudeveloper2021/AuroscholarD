...
public class PassportMonthSpinnerAdapter extends ArrayAdapter {
    StateSpinnerItemBinding binding;
    List<PassportMonthResModel> list;
    ...
    public PassportMonthSpinnerAdapter(@NonNull Context context, int resource, List<PassportMonthResModel> list, CommonCallBackListner onItemClickState) {
...
/*    public StateSpinnerUserAdapter(List<StateDataModel> list) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        binding.stateTitle.setText(list.get(position).getDateName());
        binding.stateId.setText(list.get(position).getDate());

        binding.StateList.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (onItemClickState != null) {
                    onItemClickState.commonEventListner(AppUtil.getCommonClickModel(position, Status.MONTH_CLICK, list.get(position)));
                }
