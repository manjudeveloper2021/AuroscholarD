...
public class TutionTypeSpinnerAdapter extends ArrayAdapter {
    StateSpinnerItemBinding binding;
    List<TutionData> list;
    ...
    public TutionTypeSpinnerAdapter(@NonNull Context context, int resource, List<TutionData> list, CommonCallBackListner onItemClickState) {
        ...
        binding.StateList.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (onItemClickState != null) {
                    onItemClickState.commonEventListner(AppUtil.getCommonClickModel(position, Status.TUTIONTYPE, list.get(position)));
                }
