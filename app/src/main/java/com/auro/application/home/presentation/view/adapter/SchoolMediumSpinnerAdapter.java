...
public class SchoolMediumSpinnerAdapter extends ArrayAdapter {
    StateSpinnerItemBinding binding;
    List<SchoolLangData> list;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        binding.stateTitle.setText(list.get(position).getTranslated_language());

        binding.StateList.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (onItemClickState != null) {
                    onItemClickState.commonEventListner(AppUtil.getCommonClickModel(position, Status.SCHHOLMEDIUM, list.get(position)));
                }
